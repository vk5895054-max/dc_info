...
function applyBackport(wwjsDir = DEFAULT_WWJS, patchFile = DEFAULT_PATCH) {
  ...
  withLfPatch(patchFile, lfPatch => {
    try {
      ...
      // unpatched. Windows missing DLL (exit 3221225794 / 3221225781) is also pristine — not half-patched.
      const isDllMissing = e.status === 3221225794 || e.status === 3221225781;
      if (e.code === 'ENOENT' || isDllMissing) {
        // Try git fallback; if that also fails with pristine error, let outer best-effort warn, not partialTree
        try {
          applyWithGit(wwjsDir, lfPatch);
        } catch (gitErr) {
          // If git also missing or pristine, throw non-partial error so --best-effort can warn
          if ((gitErr as any).code === 'ENOENT' || (gitErr as any).status === 3221225794 || !(gitErr as any).leftPartialTree) throw gitErr;
          throw gitErr;
        }
      } else if (e.status !== 1) {
        const detail = e.stderr ? String(e.stderr).trim() : e.message;
        throw partialTree(new Error(`\`patch\` failed (${e.code ?? `exit ${e.status}`}): ${detail}`));
          ...
          // If git also missing or pristine, throw non-partial error so --best-effort can warn
          if (gitErr.code === 'ENOENT' || gitErr.status === 3221225794 || !gitErr.leftPartialTree) throw gitErr;
          throw gitErr;
        }
  });
