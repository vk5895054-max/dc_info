...
function run(root = ROOT, spawn = spawnSync, env = process.env) {
  ...
  for (const step of steps) {
    ...
    if (reason) {
      const isBestEffort = step.args && step.args.includes('--best-effort');
      const isPatchStep = step.name.includes('whatsapp-web.js') || step.name.includes('Baileys');
      // Patch steps under --best-effort are allowed to warn on pristine trees (missing patch binary, missing DLL, Baileys-only install)
      // — they must only fail on HALF-patched trees (which set leftPartialTree). Windows missing DLL (exit 3221225794) is pristine, not partial.
      const isDllMissing = reason.includes('3221225794') || reason.includes('3221225781');
      if (isBestEffort && isPatchStep && (isDllMissing || !String(reason).includes('PARTIALLY'))) {
        console.warn(`postinstall: ${step.name} skipped — ${reason} (best-effort, tree still pristine — install continues)`);
        continue;
      }
      console.error(
