...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap {
  ...
  async getChats(id: string, opts: ListOptions = {}): Promise<ChatSummary[]> {
    ...
    try {
      ...
      if (err instanceof EngineTransportError) throw err;
      this.logger.warn(`getChats fallback for ${id}: ${err instanceof Error ? err.message : String(err)}`, { sessionId: id, action: 'getChats_fallback' });
      chats = [];
    }
    // DB fallback (previous behaviour): when engine returns empty due to transient `r: r` evaluate
    // glitch, synthesize from persisted messages to keep previous chats visible (session 9cf265... had 7 distinct).
    if (chats.length === 0 && this.messageRepository) {
      try {
        const distinct = await this.messageRepository
          .createQueryBuilder('m')
          .select('m.chatId', 'chatId')
          .addSelect('MAX(m.createdAt)', 'lastAt')
          .where('m.sessionId = :sid', { sid: id })
          .groupBy('m.chatId')
          .orderBy('lastAt', 'DESC')
          .getRawMany<{ chatId: string; lastAt: string }>();
        const byChat = new Map<string, ChatSummary>();
        if (distinct.length > 0) {
          const recent = await this.messageRepository.find({
            where: { sessionId: id } as any,
            order: { createdAt: 'DESC' },
            take: 2000,
          });
          const lastByChat = new Map<string, (typeof recent)[number]>();
          for (const m of recent) if (!lastByChat.has(m.chatId)) lastByChat.set(m.chatId, m);
          for (const row of distinct) {
            const chatId = row.chatId;
            if (!chatId || byChat.has(chatId)) continue;
            const last = lastByChat.get(chatId);
            const isGroup = chatId.endsWith('@g.us');
            const name = (last as any)?.chatName || chatId.split('@')[0];
            const kind = isGroup ? 'group' : chatId.endsWith('@lid') ? 'individual' : chatId.includes('@') ? 'individual' : 'unknown';
            byChat.set(chatId, {
              id: chatId,
              name,
              isGroup,
              kind: kind as any,
              unreadCount: 0,
              timestamp: last?.timestamp ?? Math.floor(new Date(row.lastAt).getTime() / 1000),
              lastMessage: (last?.body || '').slice(0, 80),
            } as ChatSummary);
          }
        }
        // include registry contacts so even contacts without messages appear as chats (shows ALL)
        try {
          const regs: Array<{ phone: string; name: string | null }> = await this.dataSource.query('SELECT phone, name FROM registry_contacts ORDER BY created_at DESC LIMIT 1000');
          for (const r of regs) {
        ...
        if (byChat.size > 0) {
          this.logger.log(`getChats DB fallback for ${id}: synthesized ${byChat.size} chats from messages+registry (distinct=${distinct.length})`, { sessionId: id, action: 'getChats_db_fallback' });
          chats = Array.from(byChat.values()).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        }
      } catch (e) {
        this.logger.warn(`getChats DB fallback failed for ${id}: ${String(e)}`);
      }
    }
