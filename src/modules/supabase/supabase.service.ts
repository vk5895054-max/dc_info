...
export class SupabaseService {
  ...
  // Template media: upload to Supabase Storage (bucket `templates`), fallback to inline URL/base64 if not configured
  async uploadTemplateMedia(buffer: Buffer, filename: string, mimetype: string, supabasePath?: string): Promise<{ publicUrl: string; path: string } | null> {
    if (!this.isConfigured()) return null;
    try {
      const bucket = 'templates';
      const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
      const path = supabasePath || `templates/${Date.now()}-${safeName}`;
      ...
      this.logger.warn(`uploadTemplateMedia error: ${String(e)}`);
      return null;
    }
  }

  async getTemplateMediaUrl(path: string): Promise<string | null> {
    if (!this.isConfigured() || !path) return null;
    try {
      const { data } = this.client.storage.from('templates').getPublicUrl(path);
