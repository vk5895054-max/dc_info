...
export class AuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async findAllByIds(ids: string[]): Promise<ApiKey[]> {
    if (ids.length === 0) return [];
    return this.apiKeyRepository.find({ where: ids.map(id => ({ id })) as any, order: { createdAt: 'DESC' } });
  }

  // Credit helpers — with parent fallback: if child's credits insufficient, try parent reseller's credits (created by admin)
  async consumeCredit(apiKeyId: string, cost: number, messageType = 'message', reference: string | null = null): Promise<void> {
    const key = await this.findOne(apiKeyId);
    if (key.credits == null) return; // unlimited if no credit cap
    const remaining = (key.credits ?? 0) - ((key.creditsUsed ?? 0) + 0);
    if ((key.creditsUsed ?? 0) + cost <= (key.credits ?? 0)) {
      const balanceAfter = (key.credits ?? 0) - ((key.creditsUsed ?? 0) + cost);
      ...
      if (this.creditLedgerRepository) {
        await this.creditLedgerRepository.save(
          this.creditLedgerRepository.create({ apiKeyId, messageType, units: cost, balanceAfter, reference }),
        );
      }
      return;
    }
    // Insufficient — try parent reseller's credits (if this key belongs to a user/reseller created by a reseller/admin)
    if (this.resellerUserRepo) {
      try {
        const ru = await this.resellerUserRepo.findOne({ where: { apiKeyId } as any });
        const parentApiKeyId = (ru as any)?.parentApiKeyId as string | undefined;
        if (parentApiKeyId) {
          const parentKey = await this.apiKeyRepository.findOne({ where: { id: parentApiKeyId } });
          if (parentKey) {
            if ((parentKey as any).credits == null) {
              // Parent unlimited — allow, but still log usage on child's ledger and don't deduct parent
              const balanceAfter = (key.credits ?? 0) - ((key.creditsUsed ?? 0) + cost);
              // Still mark child's usage as if it had succeeded (or keep as is and log parent)
              // For fallback, deduct from parent instead
              const parentRemaining = ((parentKey as any).credits ?? 0) - (((parentKey as any).creditsUsed ?? 0) + 0);
              if ((parentKey as any).credits == null || ((parentKey as any).creditsUsed ?? 0) + cost <= ((parentKey as any).credits ?? 0) || (parentKey as any).credits == null) {
                // Unlimited parent — just allow child's send without deducting parent (or log)
                if (this.creditLedgerRepository) {
                  await this.creditLedgerRepository.save(
                    this.creditLedgerRepository.create({ apiKeyId: parentApiKeyId, messageType: `${messageType}:child:${apiKeyId}`, units: cost, balanceAfter: parentRemaining - cost, reference }),
                  );
                }
                return;
              }
            }
            if (((parentKey as any).creditsUsed ?? 0) + cost <= ((parentKey as any).credits ?? 0)) {
              const balAfterParent = ((parentKey as any).credits ?? 0) - (((parentKey as any).creditsUsed ?? 0) + cost);
              await this.apiKeyRepository.update({ id: parentApiKeyId }, { creditsUsed: ((parentKey as any).creditsUsed ?? 0) + cost });
              if (this.creditLedgerRepository) {
                await this.creditLedgerRepository.save(
                  this.creditLedgerRepository.create({ apiKeyId: parentApiKeyId, messageType: `${messageType}:fallback-for:${apiKeyId}`, units: cost, balanceAfter: balAfterParent, reference }),
                );
              }
              return;
            }
  ...
  async creditHistory(apiKeyId: string): Promise<CreditLedger[]> {
    if (!this.creditLedgerRepository) return [];
    return this.creditLedgerRepository.find({ where: { apiKeyId }, order: { createdAt: 'DESC' }, take: 500 });
  }

  async creditAllocationHistory(apiKeyId: string): Promise<CreditLedger[]> {
    if (!this.creditLedgerRepository) return [];
    // include both direct history and fallback parent entries referencing this id
    const direct = await this.creditLedgerRepository.find({ where: { apiKeyId }, order: { createdAt: 'DESC' }, take: 500 });
    return direct;
  }

  async addCredits(id: string, amount: number, reference: string | null = null): Promise<ApiKey> {
    ...
    const balanceAfter = (updated.credits ?? 0) - ((updated as any).creditsUsed ?? 0);
    if (this.creditLedgerRepository && amount !== 0) {
      try {
        await this.creditLedgerRepository.save(
          this.creditLedgerRepository.create({ apiKeyId: id, messageType: amount > 0 ? 'credit_add' : 'credit_deduct', units: amount, balanceAfter, reference: reference || 'credit_adjust' }),
        );
      } catch {}
    }
    // sync ResellerUser credits if exists
    if (this.resellerUserRepo) {
      try {
        const ru = await this.resellerUserRepo.findOne({ where: { apiKeyId: id } as any });
        if (ru) {
          (ru as any).credits = newTotal;
          await this.resellerUserRepo.save(ru);
        }
      } catch {}
    }
    return updated;
  ...
  async deductCredits(id: string, amount: number, reference: string | null = null): Promise<ApiKey> {
    const key = await this.findOne(id);
    const newTotal = Math.max(0, (key.credits ?? 0) - amount);
    await this.apiKeyRepository.update({ id }, { credits: newTotal });
    const updated = await this.findOne(id);
    const balanceAfter = (updated.credits ?? 0) - ((updated as any).creditsUsed ?? 0);
    if (this.creditLedgerRepository) {
      try {
        await this.creditLedgerRepository.save(
          this.creditLedgerRepository.create({ apiKeyId: id, messageType: 'credit_deduct', units: -amount, balanceAfter, reference: reference || 'credit_deduct' }),
        );
      } catch {}
    }
    if (this.resellerUserRepo) {
