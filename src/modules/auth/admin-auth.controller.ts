...
class AdminLoginDto {
  ...
  @MinLength(3)
  @MaxLength(100)
  @Matches(/^[\w@.\-]+$/, { message: 'Login ID may contain letters, numbers, @ . _ -' })
  email!: string;
...
export class AdminAuthController {
  ...
  async login(@Body() dto: AdminLoginDto) {
    ...
    // A serverless/shared-host deployment may not have a readable bootstrap key file. Issue a
    // dashboard key in that case so valid email/password login remains a complete login flow.
    if (!apiKey) {
      const issued = await this.authService.createApiKey({
        name: `Dashboard Login ${admin.email}`,
