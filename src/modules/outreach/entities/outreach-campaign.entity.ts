...
export class OutreachCampaign {
  ...
  /** Optional template ID if campaign was created from a saved template */
  ...
  /** Media data for image/file messages (base64 or URL, stored as JSON) */
  @Column({ type: jsonColumnType(), nullable: true })
  mediaData!: { mediaUrl?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null;

  /** Multi-message extras: 2-4 photos (2MB each) + 1 video (10MB) + 1 file - sequential 10-15s pacing per person */
  @Column({ type: jsonColumnType(), nullable: true })
  extraMedia!: {
    images?: Array<{ url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null }>;
    video?: { url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null;
    document?: { url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null;
  } | null;
  ...
  @Column({ type: jsonColumnType(), nullable: true })
  buttons!: Array<{ type: 'url' | 'call'; text: string; value: string }> | null;

  /** Credit cost per message for this campaign */
  @Column({ type: 'int', default: 1 })
  creditCost!: number;

  /** Total credit usage for this campaign (contacts * creditCost) */
  @Column({ type: 'int', default: 0 })
  totalCredits!: number;

  /** Reseller and user assignment for credit tracking */
  @Column({ type: 'varchar', nullable: true })
  ...
  /** Who created the campaign — for admin history view (light, no heavy relations) */
  @Column({ type: 'varchar', nullable: true })
  ...
  /** Simulated reporting mode: real sending uses small bursts but reports show inflated numbers */
  @Column({ type: 'boolean', default: false })
  simulatedMode!: boolean;

  /** Target success rate for simulated reports (0-100). Default 64% */
  @Column({ type: 'int', default: 64 })
  simulatedSuccessRate!: number;

  /**
   * When simulatedMode is true, this stores the REAL small-burst distribution plan
   * that actually sends WhatsApp messages. The main `distribution` field holds the
   * inflated display plan shown to the frontend.
   */
  @Column({ type: jsonColumnType(), nullable: true })
  realDistribution!: Array<{
    sessionId: string;
    sessionName: string;
    assigned: number;
    contacts: Array<{ phone: string; name?: string }>;
