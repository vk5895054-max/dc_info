...
export class OutreachService implements OnModuleInit, OnModuleDestroy {
  ...
  private async createCampaign(dto: CreateOutreachCampaignDto): Promise<OutreachCampaign> {
    ...
    const burstProgress = this.buildBurstProgress(distribution, strategy, null);
    ...
    try {
      ...
      await this.bulkMessage.createBatch(burst.sessionId, {
        ...
        messages: burst.contacts.map(c => {
          ...
          const text = this.applyVariables(campaign.messageText, {
            ...campaign.variableMap,
            name: c.name || c.phone,
            phone: c.phone,
          });
          const caption = mediaData?.caption ? this.applyVariables(mediaData.caption, { name: c.name || c.phone, phone: c.phone }) : text;
          if ((msgType === 'image' || msgType === 'video') && mediaData) {
            // Send as native media with caption so it does NOT look bulk — like Message Tester
  ...
  async update(id: string, dto: Partial<CreateOutreachCampaignDto> & { caption?: string }): Promise<OutreachCampaignResponseDto> {
    ...
    if (contactsChanged || sessionsChanged) {
      ...
      const { sessions, missing } = await this.resolveSessionPool(sessionNames.map((s: any) => s.sessionName), warmupSchedule, (dto.strategy as any)?.maxPerSessionPerDay ?? campaign.strategy.maxPerSessionPerDay);
    ...
    // Recalculate credits if messageType changed
    if ((dto as any).messageType || dto.contacts) {
      const cost = (campaign as any).messageType === 'image' || (campaign as any).messageType === 'document' || (campaign as any).messageType === 'video' ? 2 : 1;
