...
export class TemplateController {
  ...
  @Get('history/all')
  @RequireRole(ApiKeyRole.OPERATOR)
  @ApiOperation({ summary: 'History: light list of all templates (admin sees all, reseller/user see own)' })
  async history(@CurrentApiKey() apiKey?: ApiKey, @Req() req?: Request): Promise<any> {
    const role = (apiKey as any)?.role as string | undefined;
    const isAdmin = role === 'admin' || role === 'super_admin';
    if (isAdmin) return this.templateService.history();
    const headerEmail = (req as any)?.headers?.['x-admin-email'] as string | undefined;
    const email = headerEmail || (apiKey as any)?.email || (apiKey as any)?.name || null;
    return this.templateService.history({ createdByEmail: email, role, resellerId: apiKey?.id, userId: apiKey?.id, isAdmin: false });
  }
  ...
  async findBySession(@Param('sessionId') sessionId: string, @CurrentApiKey() apiKey?: ApiKey, @Req() req?: Request): Promise<Template[]> {
    if (!apiKey) return this.templateService.findBySession(sessionId);
    const role = (apiKey as any)?.role as string | undefined;
    const isAdmin = role === 'admin' || role === 'super_admin';
