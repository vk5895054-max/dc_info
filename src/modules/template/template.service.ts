...
export class TemplateService {
  ...
  async create(sessionId: string, dto: CreateTemplateDto): Promise<Template> {
    ...
    // Handle extraMedia for mixed template (text+photo+video+file together)
    let extraMedia = (dto as any).extraMedia ?? null;
    // If extraMedia contains base64 and Supabase configured, upload each
    if (extraMedia && this.supabaseService?.isConfigured()) {
      for (const key of ['image', 'video', 'document']) {
        const m = (extraMedia as any)[key];
        if (m?.mediaBase64) {
          try {
            const buffer = Buffer.from(m.mediaBase64, 'base64');
            const filename = m.filename || `template-${key}-${Date.now()}`;
            const mimetype = m.mimetype || 'application/octet-stream';
            const uploaded = await this.supabaseService.uploadTemplateMedia(buffer, filename, mimetype);
            if (uploaded) {
              (extraMedia as any)[key].mediaUrl = uploaded.publicUrl;
              (extraMedia as any)[key].supabasePath = uploaded.path;
              (extraMedia as any)[key].mediaBase64 = null;
            }
          } catch {}
        }
      }
    }
    // Wapp Btn buttons validation
    const buttons = (dto as any).buttons ?? null;
    if (buttons) {
      for (const b of buttons) {
        if (b.type === 'url' && !/^https?:\/\/.+/.test(b.value)) throw new ConflictException(`Invalid URL for button ${b.text}: ${b.value}`);
        if (b.type === 'call' && !/^\d{10}$/.test(b.value.replace(/\D/g,''))) throw new ConflictException(`Call button requires 10 digit Indian number, got ${b.value}`);
      }
    }
  ...
  async history(filter?: { createdByEmail?: string | null; role?: string; resellerId?: string; userId?: string; isAdmin?: boolean }): Promise<Array<{ id: string; name: string; body: string; mediaType: string | null; createdByEmail: string | null; createdByRole: string | null; sessionId: string; createdAt: Date }>> {
    const all = await this.templateRepository.find({ order: { createdAt: 'DESC' } });
    let filtered = all;
    if (filter && filter.isAdmin === false) {
      filtered = all.filter(t => {
        const a: any = t as any;
        if (filter.role === 'reseller' && filter.resellerId) return a.resellerId === filter.resellerId || a.createdByEmail === filter.createdByEmail;
        if (filter.role === 'user' && filter.userId) return a.userId === filter.userId || a.createdByEmail === filter.createdByEmail;
        if (filter.createdByEmail) return a.createdByEmail === filter.createdByEmail || a.resellerId === filter.resellerId || a.userId === filter.userId;
        return true;
      });
      // if user has no own templates, also show legacy templates without creator for UX
      if (filtered.length === 0) {
        const legacy = all.filter(t => !(t as any).createdByEmail && !(t as any).resellerId && !(t as any).userId);
        if (legacy.length > 0) filtered = legacy.slice(0, 50);
      }
    }
  ...
  async update(sessionId: string, id: string, dto: UpdateTemplateDto): Promise<Template> {
    ...
    if ((dto as any).extraMedia !== undefined) (template as any).extraMedia = (dto as any).extraMedia;
    if ((dto as any).buttons !== undefined) (template as any).buttons = (dto as any).buttons;
