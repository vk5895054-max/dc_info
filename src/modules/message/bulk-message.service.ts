...
interface BulkMessageContent {
  ...
  buttons?: Array<{ type: 'url' | 'call'; text: string; value: string }>;
}
...
export class BulkMessageService implements OnApplicationBootstrap {
  ...
  async createBatch(sessionId: string, dto: SendBulkMessageDto): Promise<MessageBatch> {
    ...
    // Merge multiple items to same chatId (text+photo+video+file at once → one message)
    // WhatsApp flags 4 rapid sends to same person as spam; single media+caption is safe.
    // Enabled by default; opt-out via BULK_MERGE_PER_CHAT=false
    const mergeEnabled = (process.env.BULK_MERGE_PER_CHAT ?? 'true').toLowerCase() !== 'false';
    if (mergeEnabled && messages.length > 1) {
      const byChat = new Map<string, typeof messages>();
      for (const m of messages) {
        const key = m.chatId.trim().toLowerCase();
        const arr = byChat.get(key);
        if (arr) arr.push(m);
        else byChat.set(key, [m]);
      }
      const needsMerge = Array.from(byChat.values()).some(arr => arr.length > 1);
      if (needsMerge) {
        // Keep all distinct media for same chat as separate messages so text+photo+video+file all delivered
        // (previous version kept only best media → only video arrived). They will be sent sequentially
        // with humanized delay (30-120s) + typing, so not flagged as spam burst.
        const merged: typeof messages = [];
        for (const [, group] of byChat) {
          if (group.length === 1) { merged.push(group[0]); continue; }
          for (const it of group) merged.push(it);
        }
        this.logger.log(`Bulk per-chat grouped: ${messages.length} items → ${merged.length} kept (same chat groups kept separate media, not merged to one)`);
        messages = merged;
      }
    }
  ...
  private sendMessage(
    ...
    chatId: string,
    type: string,
    content: BulkMessageContent & { buttons?: Array<{ type: string; text: string; value: string }> },
    ...
    switch (type) {
      ...
      case 'buttons': {
        const btns = (content as any).buttons as Array<{ type: string; text: string; value: string }> | undefined;
        const text = content.text || content.caption || '';
        // Try native buttons via wwebjs if exposed, else fallback to text + links
        const anyEngine = engine as any;
        if (typeof anyEngine.sendButtonsMessage === 'function') {
          return anyEngine.sendButtonsMessage(chatId, text, btns, content.image || content.document ? { image: content.image, document: content.document, caption: content.caption } as any : undefined);
        }
        // Fallback: render buttons as text lines with URL / tel
        let fallback = text;
        if (btns?.length) {
          fallback += '\n\n';
          for (const b of btns) {
            if (b.type === 'url') fallback += `🔗 ${b.text}: ${b.value}\n`;
            else if (b.type === 'call') fallback += `📞 ${b.text}: +91 ${b.value.replace(/\D/g,'').slice(-10)}\n`;
          }
        }
        if (content.image) {
          return engine.sendImageMessage(chatId, { mimetype: content.image.mimetype || 'image/jpeg', data: stripBase64DataUri(content.image.base64) || content.image.url || '', caption: fallback.trim() } as any);
        }
        if (content.document) {
          return engine.sendDocumentMessage(chatId, { mimetype: content.document.mimetype || 'application/pdf', data: stripBase64DataUri(content.document.base64) || content.document.url || '', filename: content.document.filename, caption: fallback.trim() } as any);
        }
        return engine.sendTextMessage(chatId, fallback.trim());
      }
      case 'mixed': {
        // Legacy single-message mixed (text+photo+video+file collapsed) – pick best media to deliver at least something.
        // New campaigns use buildMessagesForBurst to expand mixed into separate image/video/document/text messages,
        // so this path is only for old `mixedtype` batches that were enqueued before the fix.
        const extra: any = (content as any).extraMedia || content;
        const cap = (content as any).caption || (content as any).text || '';
        if (extra.video || (content as any).video) {
          const v = extra.video || (content as any).video;
          return engine.sendVideoMessage(chatId, { mimetype: v.mimetype || 'video/mp4', data: stripBase64DataUri(v.base64) || v.url || '', caption: cap } as any);
        }
        if (extra.image || (content as any).image) {
          const img = extra.image || (content as any).image;
          return engine.sendImageMessage(chatId, { mimetype: img.mimetype || 'image/jpeg', data: stripBase64DataUri(img.base64) || img.url || '', caption: cap } as any);
        }
        if (extra.document || (content as any).document) {
          const doc = extra.document || (content as any).document;
          return engine.sendDocumentMessage(chatId, { mimetype: doc.mimetype || 'application/octet-stream', data: stripBase64DataUri(doc.base64) || doc.url || '', filename: doc.filename, caption: cap } as any);
        }
        return engine.sendTextMessage(chatId, cap || extra.text || (content as any).text || '');
      }
      default:
