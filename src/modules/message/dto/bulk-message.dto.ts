...
class BulkButtonDto {
  @ApiProperty({ description: 'Button type url|call', enum: ['url', 'call'] })
  @IsIn(['url', 'call'])
  type!: 'url' | 'call';

  @ApiProperty({ description: 'Button label e.g. Visit Now / Call Now' })
  @IsString()
  text!: string;

  @ApiProperty({ description: 'Value: https:// URL or 10 digit phone' })
  @IsString()
  value!: string;
}
...
class BulkMessageContentDto {
  ...
  @ApiPropertyOptional({ description: 'Wapp Btn buttons: [{type:url|call, text, value}]', type: [BulkButtonDto] })
  @IsOptional()
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => BulkButtonDto)
  buttons?: BulkButtonDto[];
}
...
class BulkMessageItemDto {
  ...
  @ApiProperty({ description: 'Message type', enum: ['text', 'image', 'video', 'audio', 'document', 'buttons'] })
...
export class SendBulkMessageDto {
  ...
  @ApiProperty({
    description:
      'Array of messages (max 500 per request; exact duplicate entries are collapsed — first occurrence wins)',
