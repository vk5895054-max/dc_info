...
export class MessageSendService {
  ...
  async sendTemplate(sessionId: string, dto: SendTemplateMessageDto): Promise<MessageResponseDto> {
    ...
    const vars = dto.vars ?? {};
    const segments = [(template as any).header, (template as any).body, (template as any).footer]
    ...
    // Flexible media template — like Message Tester: image+text / file+text, sent as native WhatsApp media so it does NOT look like bulk
    const mediaType = (template as any).mediaType as string | undefined;
    const hasMedia = mediaType && mediaType !== 'text' && ((template as any).mediaUrl || (template as any).mediaBase64 || (template as any).supabasePath);
    if (hasMedia) {
      // Resolve Supabase path to public URL if needed
      let mediaUrl = (template as any).mediaUrl as string | null;
      if (!mediaUrl && (template as any).supabasePath && this.supabaseService) {
        mediaUrl = await this.supabaseService.getTemplateMediaUrl((template as any).supabasePath);
      }
      const base64 = (template as any).mediaBase64 as string | null;
      const mimetype = (template as any).mimetype as string | null;
      const filename = (template as any).filename as string | null;
      const rawCaption = (template as any).caption as string | null;
      const caption = rawCaption ? renderTemplate(rawCaption, vars) : text; // caption wins, fallback to body
      const mediaPayload: any = {};
      if (base64) {
        mediaPayload.base64 = base64;
      ...
      if (mediaType === 'image') return this.sendImage(sessionId, { chatId: dto.chatId, ...mediaPayload });
      if (mediaType === 'video') return this.sendVideo(sessionId, { chatId: dto.chatId, ...mediaPayload });
      if (mediaType === 'document') return this.sendDocument(sessionId, { chatId: dto.chatId, ...mediaPayload });
      if (mediaType === 'audio') return this.sendAudio(sessionId, { chatId: dto.chatId, ...mediaPayload });
    }
