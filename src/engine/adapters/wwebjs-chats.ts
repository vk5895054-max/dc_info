...
export class WwebjsChats {
  ...
  async getChats(): Promise<ChatSummary[]> {
    ...
    try {
      ...
      const full = `${msg}\n${stack}`;
      // Any evaluate failure from WA Web (seen as `r: r`, `ReferenceError: r is not defined`,
      // `Evaluation failed`, `ExecutionContext was destroyed` outside the navigation grace, or
      // generic page-side TypeError from a bundle mismatch) must not become a 500 that breaks
      // the Chats nav. Treat as transient and return empty so the UI stays usable; the next
      // poll/retry will self-heal when WA Web reloads.
      this.host.logger.warn(`getChats failed, returning empty to keep UI alive: ${msg}`, {
        action: 'getChats_empty_fallback',
        error: full.slice(0, 2000),
      });
      // One best-effort retry for the classic `r: r` shape before giving up
      if (/r:\s*r|ReferenceError|Evaluation failed|ExecutionContext|TypeError/i.test(full)) {
        try {
          await new Promise(r => setTimeout(r, 800));
          chats = await this.client().getChats();
        } catch (retryErr) {
          const retryMsg = retryErr instanceof Error ? retryErr.message : String(retryErr);
          if (this.host.isPageTransportError(retryErr)) {
            this.host.reportIfPageTransportError(retryErr, 'getChats:retry');
            throw new EngineTransportError('Transport died while listing chats');
          }
          this.host.logger.warn(`getChats retry still failing, trying Store.Chat fallback: ${retryMsg}`, { action: 'getChats_store_fallback' });
          // WA Web bundle mismatch: WWebJS.getChats wrapper broken, but Store.Chat may still work.
          // Keep engine contract unchanged – just try the underlying Store directly so existing chats remain visible.
          try {
            ...
            if (pupPage?.evaluate) {
              const raw = await pupPage.evaluate(() => {
                ...
                // Try multiple store shapes: Chat.models, Chat.getModelsArray(), WWebJS.getChats fallback
                const models: any[] =
                  store?.Chat?.models ??
                  (typeof store?.Chat?.getModelsArray === 'function' ? store.Chat.getModelsArray() : []) ??
                  [];
                // Return ALL chats (no slice) so pagination in SessionService can show all
                return models.map((c: any) => ({
                  id: c?.id?._serialized ?? c?.id?._serialized ?? c?.id ?? null,
                  name: c?.name ?? c?.formattedTitle ?? c?.id?._serialized ?? c?.id ?? '',
                  isGroup: !!c?.isGroup,
                  unreadCount: c?.unreadCount ?? 0,
                  timestamp: c?.t ?? c?.timestamp ?? c?.lastMessage?.t ?? 0,
                  lastMessageBody: c?.lastMessage?.body ?? c?.lastMessage?._data?.body ?? '',
                  lastMessageType: c?.lastMessage?.type ?? c?.lastMessage?._data?.type ?? '',
                }));
              });
              if (raw && raw.length > 0) {
                chats = raw
                  ...
                  .map((r: any) => ({
                    ...
                    lastMessage: r.lastMessageType === MessageTypes.LOCATION ? '📍' : r.lastMessageBody || undefined,
                  })) as any;
              } else {
                return [];
              }
            ...
            this.host.logger.warn(`getChats Store fallback also failed, returning empty: ${String(storeErr)}`, { action: 'getChats_store_failed' });
            return [];
          }
        }
      } else {
