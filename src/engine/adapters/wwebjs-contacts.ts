...
export class WwebjsContacts {
  ...
  async getProfilePicture(contactId: string): Promise<string | null> {
    ...
    try {
      ...
      const isTransientR = /r:\s*r|ReferenceError|Evaluation failed|ExecutionContext/i.test(msg + (error instanceof Error ? error.stack ?? '' : ''));
      if (isTransientR) {
        this.host.logger.debug(`getProfilePicture transient r: r for ${contactId}, returning null`, { action: 'getProfilePicture_transient' });
        return null;
      }
