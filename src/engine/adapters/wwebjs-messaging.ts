 ...
 *
 * Historically this collapsed `!chat` (nothing sent) and `Msg.get` miss (sent, id unread) into one
 * `undefined` and threw SEND_FAILED. In production the miss is dominant and surfaces as a campaign
 * report of 0 sent / 1 failed with `the engine returned no message...` while the phone shows the
 * message delivered. Treat an absent message as sent with unknown id — `saveOutgoingMessage` leaves
 * `waMessageId` NULL so no ack is mis-matched, and the batch is counted as delivered. A true
 * unresolved-chat still reports sent here; the chat existence is independently validated via
 * `getNumberId` pre-check and the bulk pre-check, so the false-positive window is narrow and
 * visible via delivery acks, while the false-negative campaign spam is eliminated.
 */
...
export class WwebjsMessaging {
  ...
  async sendButtonsMessage(chatId: string, text: string, buttons: Array<{ type: string; text: string; value: string }>, media?: MediaInput): Promise<MessageResult> {
    this.host.ensureReady();
    // Try native Buttons (wwebjs Buttons) — best effort, fallback to text+links
    try {
      const mod: any = await import('whatsapp-web.js');
      const Buttons = mod.Buttons || mod.default?.Buttons;
      if (Buttons) {
        const btnObjs = buttons.map(b => {
          if (b.type === 'url') return { body: b.text || 'Visit Now', url: b.value };
          // call button
          const digits = b.value.replace(/\D/g, '').slice(-10);
          return { body: b.text || 'Call Now', phoneNumber: `+91${digits}` };
        });
        // If media provided, send media with caption + buttons via Buttons with media? wwebjs Buttons supports media via second arg?
        // Fallback: send text buttons; media will be sent as separate if needed
        const btnMessage = new Buttons(text, btnObjs, '', '');
        const msg = await this.sendResolved(chatId, to => this.client().sendMessage(to, btnMessage));
        return toMessageResult(msg);
      }
    } catch {}
    // Fallback — text with links (guaranteed deliverable)
    let fallback = text;
    if (buttons?.length) {
      fallback += '\n\n';
      for (const b of buttons) {
        if (b.type === 'url') fallback += `🔗 ${b.text}: ${b.value}\n`;
        else fallback += `📞 ${b.text}: +91 ${b.value.replace(/\D/g,'').slice(-10)}\n`;
      }
    }
    if (media) return this.sendMediaMessage(chatId, { ...media, caption: fallback.trim() } as any);
    const msg = await this.sendResolved(chatId, to => this.client().sendMessage(to, fallback.trim()));
    return toMessageResult(msg);
  }
  ...
  async getChatHistory(
    ...
    try {
      ...
      const isTransientR = /r:\s*r|ReferenceError|Evaluation failed|ExecutionContext/i.test(msg + (err instanceof Error ? err.stack ?? '' : ''));
      if (isTransientR) {
        this.host.logger.warn(`getChatHistory transient r: r for ${chatId}, returning empty`, { action: 'getChatHistory_transient', chatId });
        return [];
      }
      this.host.reportIfPageTransportError(err, 'getChatHistory');
      throw err;
    }
