...
export interface MessageTemplate {
  ...
  extraMedia?: Record<string, unknown> | null;
...
// ---------------------------------------------------------------------------
// Registry API types — local lead registry + blocked/reported registry + reply tracking
// ---------------------------------------------------------------------------

export interface RegistryContactItem {
  phone: string;
  name?: string;
}

export interface ImportContactsRequest {
  items: RegistryContactItem[];
  /** Skip numbers already in the session WhatsApp addressbook (engine truth). */
  checkWhatsAppAddressbook?: boolean;
  /** Mirror-save new numbers into a session WhatsApp addressbook. */
  saveToWhatsApp?: boolean;
  sessionName?: string;
  campaignId?: string;
  /** Run a live engine number-check on each candidate and DROP numbers not on WhatsApp. */
...
export interface RegistryContactRow {
  id: string;
  phone: string;
  name?: string | null;
...
export type BlockKind = 'blocked' | 'reported';
export type BlockSource = 'manual' | 'engine';

export interface RegistryBlockedRow {
  id: string;
  phone: string;
  kind: BlockKind;
  sessionName?: string | null;
...
export interface BlockedListResponse {
  items: RegistryBlockedRow[];
  engineBlocked: string[];
}

export interface SessionReplyStats {
  sessionName: string;
  sessionId: string;
  sent: number;
  replied: number;
...
// ---------------------------------------------------------------------------
// Outreach API types — multi-session round-robin campaigns + burst schedule
// ---------------------------------------------------------------------------

export type OutreachStatus = 'scheduled' | 'running' | 'completed' | 'cancelled' | 'failed';

export interface OutreachContactRef {
  phone: string;
  name?: string;
}

export interface OutreachSessionRef {
  sessionName: string;
  sessionId: string;
}
...
export interface OutreachSessionAllocation {
  sessionId: string;
  sessionName: string;
  assigned: number;
  contacts: OutreachContactRef[];
...
export interface OutreachSessionProgress {
  sessionId: string;
  sessionName: string;
  total: number;
  sent: number;
...
export interface OutreachCampaign {
  ...
  mediaData?: { mediaUrl?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null;
  extraMedia?: { images?: Array<{ url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null }>; video?: { url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null; document?: { url?: string | null; mimetype?: string | null; filename?: string | null; base64?: string | null } | null } | null;
...
export interface CreateOutreachCampaignRequest {
  ...
  templateId?: string;
  messageType?: string;
  mediaData?: { mediaUrl?: string; mimetype?: string; filename?: string; base64?: string };
  extraMedia?: { images?: Array<{ url?: string; mimetype?: string; filename?: string; base64?: string }>; video?: { url?: string; mimetype?: string; filename?: string; base64?: string } | null; document?: { url?: string; mimetype?: string; filename?: string; base64?: string } | null };
  isMulti?: boolean;
  creditCost?: number;
  totalCredits?: number;
  resellerId?: string;
...
export interface OutreachBatchExecution {
  sessionId: string;
  sessionName: string;
  batchId: string;
  status: string;
...
export interface OutreachLiveSession {
  sessionName: string;
  sessionId: string;
  totalBursts: number;
...
export interface OutreachCampaignExecution {
  campaignId: string;
  campaignName: string;
  status: OutreachStatus;
  sessionProgress?: OutreachSessionProgress[] | null;
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  const adminEmail = (() => { try { return sessionStorage.getItem('openwa_admin_email') || ''; } catch { return ''; } })();
...
export const templateApi = {
  ...
  history: (sessionId: string) => request<Array<{ id: string; name: string; body: string; mediaType: string | null; createdByEmail: string | null; createdByRole: string | null; sessionId: string; createdAt: string }>>(`/sessions/${sessionId}/templates/history/all`),
};
...
export const apiKeyApi = {
  ...
  getCreditHistory: (id: string) => request<Array<{ id: string; messageType: string; units: number; balanceAfter: number; reference?: string | null; createdAt: string }>>(`/auth/api-keys/${id}/credit-history`),
  addCredits: (id: string, amount: number) => request<ApiKey>(`/auth/api-keys/${id}/credits/add`, { method: 'POST', body: JSON.stringify({ amount }) }),
  deductCredits: (id: string, amount: number) => request<ApiKey>(`/auth/api-keys/${id}/credits/deduct`, { method: 'POST', body: JSON.stringify({ amount }) }),
...
// =============================================================================
// Registry API — local lead registry, blocked/reported registry, reply tracking
// =============================================================================

export const registryApi = {
  importContacts: (data: ImportContactsRequest) =>
    request<ImportContactsResult>('/registry/contacts/import', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  listContacts: (limit = 500) =>
    request<RegistryContactRow[]>(`/registry/contacts?limit=${limit}`),
  recordBlocked: (data: { phone: string; kind?: BlockKind; sessionName?: string; source?: BlockSource }) =>
    request<RegistryBlockedRow>('/registry/blocked', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  listBlocked: (includeEngine = true) =>
    request<BlockedListResponse>(`/registry/blocked?includeEngine=${includeEngine}`),
  removeBlocked: (phone: string, kind?: BlockKind) =>
    request<{ removed: boolean }>(`/registry/blocked/${encodeURIComponent(phone)}${kind ? `?kind=${kind}` : ''}`, {
      method: 'DELETE',
    }),
  sessionReplies: () => request<SessionReplyStats[]>('/registry/replies'),
};

// =============================================================================
// Outreach API — multi-session round-robin campaigns + burst schedule
// =============================================================================

export const outreachApi = {
  list: () => request<OutreachCampaign[]>('/outreach/campaigns'),
  get: (id: string) => request<OutreachCampaign>(`/outreach/campaigns/${encodeURIComponent(id)}`),
  create: (data: CreateOutreachCampaignRequest) =>
    request<OutreachCampaign>('/outreach/campaigns', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  update: (id: string, data: Partial<CreateOutreachCampaignRequest>) =>
    request<OutreachCampaign>(`/outreach/campaigns/${encodeURIComponent(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  start: (id: string) =>
    request<OutreachCampaign>(`/outreach/campaigns/${encodeURIComponent(id)}/start`, { method: 'POST' }),
  stop: (id: string) =>
...
export const panelApi = {
  getHours: () => request<{ enabled: boolean; startHour: number; endHour: number; timezone?: string }>('/panel/hours'),
  check: () => request<{ enabled: boolean; startHour: number; endHour: number; timezone?: string; blocked: boolean; role: string | null }>('/panel/hours/check'),
  setHours: (data: { enabled: boolean; startHour: number; endHour: number }) =>
    request<{ enabled: boolean; startHour: number; endHour: number }>(`/panel/hours`, { method: 'PUT', body: JSON.stringify(data) }),
  createRequest: (data: { requestedHours: string; reason?: string }) => request<any>(`/panel/requests`, { method: 'POST', body: JSON.stringify(data) }),
  listRequests: () => request<any[]>(`/panel/requests`),
  listMyRequests: () => request<any[]>(`/panel/requests/me`),
  updateRequest: (id: string, data: { status: 'approved' | 'rejected'; grantedHours?: number }) => request<any>(`/panel/requests/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
};
