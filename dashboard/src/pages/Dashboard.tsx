...
import { MessageSquare, Send,  Activity, Loader2, Coins, Search } from 'lucide-react';
...
import { PanelHoursCard } from '../components/PanelHoursCard';
...
export function Dashboard() {
  ...
  const filteredSessions = (sessions as any[]).filter(s => {
    ...
    if (!q) return true;
    return [s.name, s.id, s.phone || '', s.status].some(v => String(v).toLowerCase().includes(q));
  });
  ...
  // Hide connected-session card for reseller/user as requested
  const statsCards = isAdmin ? statsCardsAll : statsCardsAll.filter(c => c.label !== t('dashboard.stats.activeSessions'));
  ...
  return (
    <div className="dashboard">
      ...
      />
      {isAdmin && <PanelHoursCard />}  

      {(isDemo || isReseller || isUser) && currentKey && (
        <div style={{ border: '1px solid #333', borderRadius: 12, padding: 16, background: '', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <Coins size={24} />
          <div style={{ flex: 1 }}>
      ...
      {isAdmin && (
        ...
        <div className="section-header">
          ...
          <span className="section-subtitle">
            {t('dashboard.showingSessions', { shown: filteredSessions.length, total: stats?.total ?? 0 })}
          </span>
        </div>
        ...
        <div style={{ position:'relative', marginBottom:12, maxWidth:380 }}>
          ...
          <input
            ...
            placeholder={`${t('common.search')} sessions, phone, status...`}
        ...
        <div className="sessions-table">
          ...
          {filteredSessions.length === 0 ? (
            <div className="table-row" style={{ justifyContent: 'center', color: 'var(--text-muted)' }}>
              {sessions.length===0 ? t('dashboard.noSessions') : `No sessions match "${sessionSearch}"`}
            </div>
          ) : (
