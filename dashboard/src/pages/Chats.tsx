...
import { ArrowLeft, Loader2, Megaphone, CircleDashed, AlertCircle, MessageSquare, Filter } from 'lucide-react';
...
export function Chats() {
  ...
  // Campaign filter for campaign-wise replies
  const [campaignFilter, setCampaignFilter] = useState<string>('');
  const { data: campaigns = [] } = useQuery({
    queryKey: ['outreach-history-chats'],
    queryFn: outreachApi.history,
    staleTime: 30_000,
  });
  const selectedCampaign = useMemo(() => (campaigns as any[]).find((c:any)=> c.id===campaignFilter) || null, [campaigns, campaignFilter]);
  ...
  // 3. WebSocket integration for real-time messages – campaign replies (any text) must surface
  const handleIncomingMessage = useCallback(
    (event: { sessionId: string; message: Record<string, unknown> }) => {
      // Do not drop messages for other sessions – campaign may have used a different session
      const isOtherSession = event.sessionId !== selectedSessionId;
      ...
      if (!isOtherSession && activeChat && newMsg.chatId === activeChat.id) {
        markChatRead(activeChat.id);
        if (!newMsg.fromMe) onMessageAppended('incoming');
      }

      // For other sessions (campaign used different number), show a toast so reply is visible even
      // when Chats is filtered to a single session. Any text (not just "interested") counts.
      if (isOtherSession && !newMsg.fromMe) {
        const sessName = sessions.find(s => s.id === event.sessionId)?.name ?? event.sessionId.slice(0, 8);
        showWarningToast(`New reply on ${sessName}: ${newMsg.body?.slice(0, 60) || '(media)'}`, `From ${newMsg.chatId.split('@')[0]}`);
      }

      // Update sidebar chat list. For selected session, apply reducer; for other sessions, keep as is
      // but still trigger a subtle refetch toast above. For unknown chats, insert directly so
      // `r: r` engine fallback (DB synthesized) still shows the new reply instantly without waiting
      // for `loadChats` which may be empty due to WA-Web `r: r` glitch.
      if (!isOtherSession) {
        ...
        setChats(prevChats => {
          ...
          if (needsSidebarRefetch) {
            // Direct insert instead of refetch when engine is in `r: r` fallback (would return same 7)
            const exists = prevChats.some(c => c.id === newMsg.chatId);
            if (!exists) {
              const newChat: Chat = {
                id: newMsg.chatId,
                name: (newMsg as any).contact?.name || (newMsg as any).contact?.pushName || newMsg.chatId.split('@')[0],
                isGroup: newMsg.chatId.endsWith('@g.us'),
                kind: (newMsg.chatId.endsWith('@g.us') ? 'group' : 'individual') as ChatKind,
                unreadCount: newMsg.fromMe ? 0 : 1,
                timestamp: newMsg.timestamp,
                lastMessage: newMsg.body?.slice(0, 80),
              };
              insertedDirectly = true;
              return [newChat, ...result.chats];
            }
          }
          return result.chats;
        });
        // Only refetch if we didn't already insert and engine is healthy; otherwise the inserted row suffices
        if (needsSidebarRefetch && !insertedDirectly) {
          // slight delay lets message table persist before DB fallback synthesizes
          setTimeout(() => void loadChats(selectedSessionId), 400);
        }
      }
    },
    [selectedSessionId, activeChat, loadChats, markChatRead, appendMessage, onMessageAppended, t, sessions, showWarningToast],
  );
  ...
  useEffect(() => {
    ...
    // Subscribe to ALL ready sessions so campaign replies on any session surface in Chats,
    // not just the currently selected one. Unsubscribe on cleanup per session.
    const ids = sessions.length > 0 ? sessions.map(s => s.id) : selectedSessionId ? [selectedSessionId] : [];
    ...
    return () => {
      for (const sid of ids) unsubscribe(sid);
    };
  }, [sessions, selectedSessionId, isConnected, subscribe, unsubscribe]);
  ...
  // Campaign replies for campaign-wise filter (session-wise + interest)
  const { data: campReplies } = useQuery({
    queryKey: ['outreach', campaignFilter, 'replies'],
    queryFn: () => outreachApi.replies(campaignFilter),
    enabled: Boolean(campaignFilter),
    staleTime: 15_000,
  });
  ...
  const filteredChatsBase = filterChats(chats, searchQuery);
  const filteredChats = useMemo(() => {
    if (!campaignFilter || !selectedCampaign) return filteredChatsBase;
    const phones = new Set<string>(((selectedCampaign.contacts as any[]) ?? []).map((c:any)=> String(c.phone).replace(/\D/g,'')));
    // campaign replies set (handles @lid vs @c.us via direct chatId + phone suffix) – any reply text counts
    const replyChatIds = new Set<string>(((campReplies as any)?.replies ?? []).map((r:any)=> String(r.chatId)));
    const replyPhones = new Set<string>(((campReplies as any)?.replies ?? []).map((r:any)=> String(r.phone).replace(/\D/g,'')));
    const hasRepliesData = (campReplies as any)?.replies?.length > 0;
    return filteredChatsBase.filter(chat => {
      const digits = chat.id.split('@')[0].replace(/\D/g,'');
      // inCampaign: either phone suffix matches OR this chat is a direct reply chatId (covers @lid where digits don't match phone)
      const inCampaignByPhone = Array.from(phones).some((p:string) => digits.endsWith(p) || p.endsWith(digits));
      const inCampaignByReply = replyChatIds.has(chat.id);
      const inCampaign = inCampaignByPhone || inCampaignByReply;
      if (!inCampaign) return false;
      // when campaignFilter is active, show only chats that actually replied after campaign start (any text)
      // – any reply (hi, interested, not interested, dont msg me, etc.) counts, matching user request
      if (hasRepliesData) {
        const hasReply = replyChatIds.has(chat.id) || Array.from(replyPhones).some((p:string) => digits.endsWith(p) || p.endsWith(digits));
        return hasReply;
      }
      // before replies data loads, show all campaign contacts so user sees who was targeted
      return true;
    });
  }, [filteredChatsBase, campaignFilter, selectedCampaign, campReplies]);

  // Ensure campaign replies that have not yet appeared in the engine chat list (e.g. @lid chats not in Store fallback)
  // are still visible in the filtered view by synthesizing them from replies data.
  useEffect(() => {
    if (!campaignFilter || !campReplies?.replies?.length) return;
    const existing = new Set(chats.map(c => c.id));
    const missing = (campReplies.replies as any[]).filter(r => !existing.has(r.chatId));
    if (missing.length === 0) return;
    const synth: Chat[] = missing.map((r: any) => ({
      id: r.chatId,
      name: r.name || r.phone,
      isGroup: false,
      ...
      lastMessage: r.body?.slice(0, 80),
    }));
    setChats(prev => [...synth, ...prev]);
  }, [campReplies, campaignFilter]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="chats-page">
      ...
      {/* Campaign-wise reply filter: show only chats from campaign contacts, grouped session-wise */}
      {!loadingSessions && sessions.length > 0 && (
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:12, padding:'10px 12px', background:'var(--bg-white)', border:'1px solid var(--border)', borderRadius:10}}>
          <Filter size={16} style={{color:'var(--text-muted)'}} />
          <span style={{fontSize:13, fontWeight:600, color:'var(--text-primary)'}}>Campaign filter:</span>
          <select value={campaignFilter} onChange={e=> setCampaignFilter(e.target.value)} style={{minWidth:200, padding:'6px 10px', border:'1px solid var(--border)', borderRadius:8, fontSize:13}}>
            <option value="">All chats</option>
            {(campaigns as any[]).map((c:any)=> <option key={c.id} value={c.id}>{c.name} — {c.contactCount} contacts · {c.status}</option>)}
          </select>
          {campaignFilter && selectedCampaign && <span style={{fontSize:12, color:'var(--text-muted)'}}>{filteredChats.length} chats in campaign</span>}
          {campaignFilter && campReplies && (
            <span style={{display:'flex',gap:6, marginLeft:8, flexWrap:'wrap'}}>
              <span style={{padding:'4px 8px',borderRadius:999,background:'#22c55e',color:'#fff',fontSize:11}}>Interested {campReplies.summary.interested}</span>
              <span style={{padding:'4px 8px',borderRadius:999,background:'#ef4444',color:'#fff',fontSize:11}}>Not {campReplies.summary.notInterested}</span>
              <span style={{padding:'4px 8px',borderRadius:999,background:'#64748b',color:'#fff',fontSize:11}}>Other {campReplies.summary.other}</span>
              <span style={{padding:'4px 8px',borderRadius:999,background:'#fff',border:'1px solid var(--border)',fontSize:11}}>Replied {campReplies.summary.replied}/{campReplies.summary.totalContacts}</span>
            </span>
          )}
          {campaignFilter && <button onClick={()=> setCampaignFilter('')} style={{marginLeft:'auto', padding:'6px 10px',borderRadius:8,border:'1px solid var(--border)',background:'var(--bg-white)',fontSize:12,cursor:'pointer'}}>Clear</button>}
        </div>
      )}
      {campReplies && campaignFilter && (
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:10}}>
          {campReplies.bySession.map(s=> (
            <span key={s.sessionId} style={{padding:'5px 10px',borderRadius:999,background:'var(--bg-secondary)',border:'1px solid var(--border)',fontSize:11}}>
              <strong>{s.sessionName}</strong>: {s.replied} replied · <span style={{color:'#22c55e'}}>{s.interested} interested</span> · <span style={{color:'#ef4444'}}>{s.notInterested} not</span> — direct send available in chat
            </span>
          ))}
        </div>
      )}
