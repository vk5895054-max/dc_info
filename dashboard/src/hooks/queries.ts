...
export function useOutreachExecutionQuery(id: string, shouldPoll: boolean) {
  return useQuery({
    ...
    staleTime: shouldPoll ? 5000 : 30_000,
  });
}
...
export function useOutreachUpdateMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (params: { id: string; data: Partial<CreateOutreachCampaignRequest> }) => outreachApi.update(params.id, params.data),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.outreach });
    },
  });
...
export function useCreditHistoryQuery(id: string, enabled = true) {
  return useQuery({
    queryKey: queryKeys.creditHistory(id),
    queryFn: () => apiKeyApi.getCreditHistory(id),
    enabled: enabled && !!id,
    staleTime: 15_000,
  });
}
